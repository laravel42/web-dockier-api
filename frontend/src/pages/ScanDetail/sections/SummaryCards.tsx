...
export default function SummaryCards({ summary, severityFilter, onFilterChange }: Props) {
  const cards = [
    { label: "Files", value: `${summary.filesScanned ?? 0}/${summary.filesInRepo ?? 0}`, color: "text-text-muted", filter: null },
    ...
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-6">
      {cards.map((c) => (
        <button
          ...
          className={`${cardCls} px-3 py-2.5 text-center transition-all ${c.filter !== null && severityFilter === c.filter ? "ring-2 ring-primary-500/40" : "hover:bg-card/60"} ${c.filter === null ? "cursor-default" : ""}`}
        >
          <p className={`${typeStatValueSm} ${c.color}`}>{c.value}</p>
