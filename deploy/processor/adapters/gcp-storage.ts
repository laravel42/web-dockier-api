      ...
      await pollUntil({
        check: async (attempt) => {
          try {
            ...
            if (res.ok) {
              await appendLog(`✓ CDN is live (attempt ${attempt})`);
              return true;
            }
            await appendLog(`⏳ CDN not ready — HTTP ${res.status} (attempt ${attempt})`);
          } catch {
            await appendLog(`⏳ CDN not reachable yet (attempt ${attempt})`);
          }
          return null;
        },
        intervalMs: 10_000,
