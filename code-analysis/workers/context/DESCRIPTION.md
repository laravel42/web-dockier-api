...
- **Security scanning** — Semgrep (community rules, 30+ languages), optional SonarQube, and a built-in custom regex rules engine (rules stored in `custom_rules`, seeded by the backend). Covers SQL injection, XSS, command injection, weak crypto, path traversal, and more.
...
- Semgrep/Opengrep rules + database-backed custom regex rules cover the OWASP Top 10 vulnerability classes across PHP, JavaScript, TypeScript, Python, Go, Java, and Ruby.
