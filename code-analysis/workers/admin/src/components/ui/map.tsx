...
function ensureStyleProjection(style: MapLibreGL.StyleSpecification) {
  if (style.projection) return style
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const [resolvedStyle, setResolvedStyle] = useState<MapStyleOption | null>(null)
  const currentStyleRef = useRef<MapStyleOption | null>(null)
  const resolvedStyleRef = useRef<MapStyleOption | null>(null)
  const styleCacheRef = useRef(
    new globalThis.Map<string, MapLibreGL.StyleSpecification>()
  )
  ...
  const clearProjectionFrame = useCallback(() => {
  ...
  useEffect(() => {
    ...
    const resolveStyle = async () => {
      if (typeof activeStyle !== "string") {
        setResolvedStyle(ensureStyleProjection(activeStyle))
        return
      }

      const cachedStyle = styleCacheRef.current.get(activeStyle)
      if (cachedStyle) {
        setResolvedStyle(cachedStyle)
        return
      }

      try {
        const response = await fetch(activeStyle, {
        ...
        const styleJson =
          ensureStyleProjection(
            (await response.json()) as MapLibreGL.StyleSpecification
          )

        styleCacheRef.current.set(activeStyle, styleJson)
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      const initialStyle = resolvedStyleRef.current

      if (!containerRef.current || !initialStyle) {
        initFrameId = window.requestAnimationFrame(initializeMap)
        return
      }
      ...
      const styleLoadHandler = () => setIsStyleLoaded(false)
      const idleHandler = () => {
  ...
  // Handle style change
  useEffect(() => {
    if (!mapInstance || !resolvedStyle) return
  ...
  const isMapReady = isLoaded && isStyleLoaded

  // Expose the map instance only when the map and style are both ready.
  useImperativeHandle(
    ref,
    () => (isMapReady ? mapInstance : null) as MapLibreGL.Map,
    [isMapReady, mapInstance]
  )

  useEffect(() => {
    if (!mapInstance || !isMapReady || !projection) return

    const desiredProjection = projection.type ?? "mercator"
    const currentProjection = mapInstance.getProjection()?.type ?? "mercator"

    if (desiredProjection === currentProjection) return

    let cancelled = false

    const applyProjection = (attemptsRemaining = 6) => {
      ...
      try {
        ...
        projectionFrameRef.current = window.requestAnimationFrame(() => {
          applyProjection(attemptsRemaining - 1)
        })
      }
    }
  ...
  }, [mapInstance, isMapReady, projection, clearProjectionFrame])
  ...
  return (
    <MapContext.Provider value={contextValue}>
      ...
      >
        {(!isMapReady || loading) && <DefaultLoader />}
        {/* Mount children only after the map and style are both ready. */}
        {isMapReady && mapInstance && children}
      </div>
    </MapContext.Provider>
  )
})
