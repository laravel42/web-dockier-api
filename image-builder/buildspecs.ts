...
function pythonDockerfile(stack: Extract<DetectedStack, { runtime: "python" }>): string {
  ...
  if (stack.framework === "django") {
    // Install Django if missing; try to detect required version from imports
  ...
  if (stack.framework === "django") {
    ...
    // Run the pre-written deploy patch script (injected by bundleAndUploadSource)
