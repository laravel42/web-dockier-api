/**
 * Run all SQL seeders + CSV imports.
 *
...
async function seedCsv(client: pg.Client, table: string, csvPath: string) {
  ...
  if (count.rows[0].n > 0) {
    console.log(`  ⏭ ${table} already has ${count.rows[0].n} rows, skipping`);
    return;
  }
...
async function main() {
  ...
  // Run SQL seeders (skip placeholder SQL files for CSV imports)
  const sqlFiles = readdirSync(seedersDir).filter(f => f.endsWith(".sql")).sort();
  console.log(`Running SQL seeders...`);
