...
**Deployment automation.** Repo analyzer detects runtime, framework, and package manager to automatically select the optimal build strategy (Nixpacks for zero-config, Railpack for Railway-compatible stacks, Dockerfile when one exists in the repo, or static for SPAs). Deploys are orchestrated via a self-hosted Dokploy instance that handles container building, Traefik routing, and TLS termination on remote servers. Compute is provisioned per project **on the user's own connected AWS or GCP account** — the pipeline launches a VPS (EC2 or Compute Engine), waits for SSH, and registers it as a Dokploy remote server; Dockier owns no shared deploy infrastructure. When a deploy fails, Dokploy's built-in AI diagnoses the failure logs and applies fixes automatically, retrying up to 3 times. Deploy wizard covers provider selection, plan (instance size), region, and environment. The pipeline shows real-time stage progress: Project → Git Sync → Server → App Config → Deploy. Deleting a project (or an explicit teardown) removes the Dokploy application and remote server and terminates the provisioned VPS on the user's account. Legacy CloudFormation/Pulumi pipeline remains available via `DEPLOY_PROVIDER=native` feature flag.
