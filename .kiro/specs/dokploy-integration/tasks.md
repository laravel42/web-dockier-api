...
- [x] 7. Provision Server Stage — per-tenant VPS on the user's own AWS/GCP account (AWS + GCP provisioning + teardown done)
  ...
  - [x] 7.18 Teardown (`lifecycle/dokploy-teardown.ts`): on project infra teardown, remove the Dokploy application (`application.remove`) + server (`server.remove`), terminate the tenant's cloud VM (`terminateEc2Instance` / `terminateGceInstance`), and delete the `dokploy_*` mapping rows. Best-effort/idempotent — per-resource results aggregate to torn_down/partial/nothing_to_tear_down. Dispatched from `teardownProjectInfrastructure` when `DEPLOY_PROVIDER=dokploy`, and also run best-effort on `deleteProject`. Unit tests: `__tests__/dokploy-teardown.test.ts`
  - [x] 7.19 Cost attribution: provisioned VMs are tagged/labeled `dockier-managed`/`dockier-project`/`dockier-tenant` (EC2 tags; GCE labels sanitized to GCP rules) so orphaned instances are traceable. `tenantId` threaded pipeline → stage → provisioner.
  - [x] 7.20 Preflight/dry-run command (`scripts/dokploy-preflight.ts`, `pnpm --filter @dockier/backend-fastify preflight:dokploy`): read-only checks of config, Dokploy reachability + auth (`project.all`), and `DOKPLOY_SSH_KEY_ID` resolution (`sshKey.all`). Fast-fail low-retry client. Run this first when Dokploy access is available.
